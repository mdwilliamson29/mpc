    ...
    if (len(H) != 0):
        for (a, b) in H:
            ...
            for (a2, b2) in T:
                for (a3, b3) in H:
                    if (a3, b3) != (a, b):
                        temp += 'w[' + str(wEdges.index(((a3, b3), (a2, b2))) + 1) + '] + '
            ampl.eval('subject to const' + str(ind) + ': x[' + str(xEdges.index((a, b)) + 1) + '] + ' + temp[:-3] + ' <= 1;')
            ind += 1"""

### Cut III ###
"""for i in range(n):
    for (a, b) in g.edges:
        T = [(a2, b2) for (a2, b2) in g.edges if a2 == i]
        temp = ''
        for (a2, b2) in T:
            if ((a, b), (a2, b2)) in wEdges:
                temp += 'w[' + str(wEdges.index(((a, b), (a2, b2))) + 1) + '] + '
        if temp != '':
            ampl.eval('subject to const' + str(ind) + ': ' + temp[:-3] + ' <= x[' + str(xEdges.index((a, b)) + 1) + '];')
            ind += 1
    print("Node " + str(i) + ", edge (" + str(a) + ", " + str(b) + ")")"""
...
"""for i in range(n):
    for (a, b) in g.edges:
        H = [(a2, b2) for (a2, b2) in g.edges if b2 == i]
        temp = ''
        for (a2, b2) in H:
            if (a2, b2) != (a, b):
                for ((c, d), (x, y)) in wEdges:
                    if x == a2 and y == b2:
                        temp += 'w[' + str(wEdges.index(((c, d), (a2, b2))) + 1) + '] + '
        ampl.eval('subject to const' + str(ind) + ': x[' + str(xEdges.index((a, b)) + 1) + '] + ' + temp[:-3] + ' <= 1;')
        ind += 1"""

"""for (a2, b2) in H:
            if (a2, b2) != (a, b):
                for (a3, b3) in g.edges:
                    temp += 'w[' + str(wEdges.index(((a3, b3), (a2, b2))) + 1) + '] + '"""
...
print("NCCD_Cuts")
ampl.eval('minimize totalCost_Cuts: 0;')
startTime = time.time()
ampl.solve()
print("ampl.solve() took " + str(time.time() - startTime) + " seconds!")
#ampl.eval('display _varname, _var.iis, _conname, _con.iis;')
totalCost = ampl.getObjective('totalCost_Cuts')
print("Objective is:", totalCost.value())


print("SNCC_Cuts")
ampl.eval('minimize totalCost_SNCC_Cuts: sum {p in 1..' + str(len(g.edges)) + '} x[p];')
startTime = time.time()
ampl.solve()
print("ampl.solve() took " + str(time.time() - startTime) + " seconds!")
#ampl.eval('display _varname, _var.iis, _conname, _con.iis;')
totalCost = ampl.getObjective('totalCost_SNCC_Cuts')
print("Objective is:", totalCost.value())
...
for ((a, b), (c, d)) in wEdges:
    if wVals[wEdges.index(((a, b), (c, d)))] == 1:
        dot.edge(str(a + 1), str(b + 1), color=colors[1])#, style='filled')
        dot.edge(str(c + 1), str(d + 1), color=colors[1], label=str(cost[wEdges.index(((a, b), (c, d)))]))
        #dot.node(str(b + 1), color=colors[0], style='filled')
        #if (a, b) not in subgraph[0] and (a, b) not in subgraph[1] and (a, b) not in subgraph[2] and (a, b) not in subgraph[3]:
    #else:
    #    dot.edge(str(a + 1), str(b + 1), color='black', style='dashed')
